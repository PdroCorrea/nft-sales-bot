...
const runSalesBot = async () => {
    ...
    let bootupDate = new Date(2022, 1, 11, 6, 35, 0, 0);
...
;
        for (let i = signatures.length - 1; i >= 0; i--) {
            try {
                ...
                console.log(`signatureDate date ${signatureDate.toLocaleString()} bootupdate date ${bootupDate.toLocaleString()}` );
                ...
                if (signatureDate > bootupDate){
                    ...
                    if (price > 0.008) {
                        if (marketplaceMap[marketplaceAccount] === 'Magic Eden') {
                            ...
                            logMetadata.txn = txn;
                            logMetadata.metadata = metadata;
                            transLogs.push(logMetadata);
                        ...
                        console.log(`small transaction value, probably listing  cost ${price}`);
                    }
            }
            else{
                console.log("skipping transaction date => ",signatureDate.toLocaleString());
                continue;
            }
            } catch (err) {
        ...
        fs.writeFile (`transactionLogs.json`, JSON.stringify(transLogs,2,null), function(err) {
            if (err) throw err;
